...
h = semilogy(tt,STi_15gt(I_STi(1:5),:),'linewidth',1); 
hold on 
...
h = semilogy(tt,STi_15gt(I_STi(6:15),xx),'linewidth',1); 
hold on 
plot(xlimits,eta1*ones(2,1),'k--',xlimits,eta2*ones(2,1),'k--')
plot(Tnew(ts)*ones(2,1),ylimits,'k-.',Tnew(t4)*ones(2,1),ylimits,'k-.')
plot(xlimits,zeros(2,1),'k')
...
h = semilogy(tt,STi_15gt(I_STi(16:length(I_STi)),xx),'linewidth',1); 
hold on 
...
if printon == 1
    ...
    print(hfig11,'-depsc2','STiLM_mid.eps')
    print(hfig12,'-depsc2','STiLM_least.eps') 
    
    print(hfig10,'-dpng','STiLM_most.png')
    print(hfig11,'-dpng','STiLM_mid.png')
    print(hfig12,'-dpng','STiLM_least.png') 
end 